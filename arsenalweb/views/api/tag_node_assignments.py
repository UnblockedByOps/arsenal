...
def api_tag_node_assignments_schema(request):
    """Schema document for tag_node_assignments API"""
...
def api_tag_node_assignment_read_attrib(request):
    """Process read requests for /api/tag_node_assignments/{id}/{resource} route matches"""
...
def api_tag_node_assignment_read_id(request):
    """Process read requests for /api/tag_node_assignments/{id} route matches"""
...
def api_tag_node_assignment_read(request):
    """Process read requests for /api/tag_node_assignments route match"""
    ...
    try:
        ...
        s = ''
        ta = DBSession.query(TagNodeAssignment)
        if any((tag_id, node_id)):
            ...
            for k,v in request.GET.items():
                ...
                if exact_get:
                    ...
                    ta = ta.filter(getattr(TagNodeAssignment ,k)==v)
                else:
                    log.info('Loose filtering on {0}={1}'.format(k, v))
                    ta = ta.filter(getattr(TagNodeAssignment ,k).like('%{0}%'.format(v)))

            log.info('Searching for tag_node_assignments with params: {0}'.format(s.rstrip(',')))

        else:
            log.info('Displaying all tag_node_assignments')
...
def api_tag_node_assignments_write(request):
    """Process write requests for /api/tag_node_assignments route match"""
    ...
    try:
        ...
        try:
            ...
            log.info('Checking for tag_node_assignment tag_id={0}node_id={1}'.format(tag_id, node_id))
            q = DBSession.query(TagNodeAssignment)
            q = q.filter(TagNodeAssignment.tag_id==tag_id)
            q = q.filter(TagNodeAssignment.node_id==node_id)
            q.one()
        ...
        log.error('Error with tag_node_assignment API! exception: {0}'.format(e))
        return Response(str(e), content_type='application/json', status_int=500)
...
def api_tag_node_assignments_delete_id(request):
    """Process delete requests for /api/tag_node_assignments/{id} route match."""
...
def api_tag_node_assignments_delete(request):
    """Process delete requests for /api/tag_node_assignments route match.
