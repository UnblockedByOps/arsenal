...
def api_tag_node_assignment_read(request):
    ...
    try:
        ...
        if any((tag_id, node_id)):
            ...
            log.debug('Searching for tag_node_assignments with params: {0}'.format(s.rstrip(',')))

        else:
            log.debug('Displaying all tag_node_assignments')
        ...
        log.error('Error reading from tag_node_assignments API={0},exception={1}'.format(request.url, e))
